...
GERMAY_PRESENTATION_PATH = os.path.join(BASE_DIR, "Main Presentation Germany.pdf")
...
with col1:
    ...
    main_message = st.text_area("Основной посыл / Оффер:", height=120, placeholder="Например: Скидка 20% на новые туры для корпоративных клиентов. ИЛИ опишите конкретный посыл, например 'Экономия'.")
    
    # Platform Selection with 2026 Context
    platform_options = {
        "Meta (Facebook/Instagram)": "Акцент на визуал, эмоции, короткие тексты. Лимит заголовка ~40 симв.",
        "LinkedIn": "Профессиональный тон, B2B фокус. Лонгриды приветствуются.",
        "Google Ads (Search)": "Строгие лимиты: Заголовок 30 симв, Описание 90 симв. (Лимиты 2026)",
    ...
    framework = st.selectbox("Маркетинговый фреймворк", [
        "Без фреймворка (Свободный стиль)",
...
with col2:
    ...
    c1, c2 = st.columns(2)
    with c1:
        language = st.selectbox("Язык", ["Русский", "Английский", "Немецкий"])
    with c2:
        geo = st.selectbox("ГЕО", ["Россия", "США", "ОАЭ", "Казахстан", "Германия", "Весь мир"])
        
    length_option = st.radio("Длина текста", ["Короткий / Емкий", "Длинный / Подробный"], horizontal=True)
...
if generate_btn:
    if not HAS_GENAI:
        ...
        try:
            ...
            prompt = f"""
            Ты - ведущий B2B копирайтер Tumodo.
            
            ЗАДАЧА: Напиши ровно 4 варианта рекламного объявления для платформы: {platform}.
            Раздели их СТРОГО строкой "====SEPARATOR====". Ничего не пиши до первого варианта и после последнего.
            
            ПАРАМЕТРЫ:
            Язык: {language}. Гео: {geo}. Фреймворк: {framework}. Длина: {length_option}.
            
            ПОСЫЛ ОТ ПОЛЬЗОВАТЕЛЯ: "{main_message}"
            --- ПРАВИЛО ПОСЫЛА ---
            Если пользователь ввел свой посыл (например 'Экономия' или конкретный оффер), ты ОБЯЗАН делать акцент строго на этом смысле во всех текстах.
            Если же посыл пустой или слишком общий — предложи свои варианты, которые по-твоему лучше всего сработают для этой аудитории ({geo}), опираясь на сильные стороны Tumodo из Бренд-платформы.
            
            ОСОБЕННОСТИ 4-ГО ВАРИАНТА:
            Вариант 4 должен быть "Best SEO & Market Fit" — используй лучшие ключевые слова, инсайты о рынке и самую эффективную структуру, независимую от выбранного фреймворка, чтобы пробить баннерную слепоту. Добавь в начале 4-го варианта кратно почему он сработает.
            ...
            УБЕДИСЬ, ЧТО:
            - Ты учитываешь строгие лимиты для платформы {platform}.
            - Ты разделил текст 4-мя строками "====SEPARATOR====".
            """
